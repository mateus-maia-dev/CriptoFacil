...
def populate_accounting(user_id):
    ...
    transaction: Transaction = Transaction.query.filter_by(
        user_id=user_id).order_by(Transaction.id.desc()).first()

    price_brl = transaction.price_per_coin if transaction.fiat == 'brl' else transaction.price_per_coin * 5
    ...
    if transaction.type == 'sell' or transaction.type == 'input':
        ...
        profit = (price_brl - transaction.avg_price_brl) * \
